...
- **Chrome Extension**：可选扩展组件，用于补充浏览器侧能力
...
- **Iframe 基础探测与同源 frame 操作**：支持 `browser_frames` 与 `frameIndex`
...
- Chrome Extension 文档：[`chrome-extension/README.md`](./chrome-extension/README.md)
...
- Same-origin iframe inspection and `frameIndex` operations
...
- Работа с same-origin iframe через `browser_frames` и `frameIndex`
