...
- **Iframe 基础探测与同源 frame 操作**：支持 `browser_frames` 与 `frameIndex`；其中 `frameIndex` 仅选择当前顶层页面中的同源 iframe，顶层 document 请省略该参数
...
- Same-origin iframe inspection and `frameIndex` operations; `frameIndex` selects same-origin iframes inside the current top-level document, and top-level document access should omit it
...
- Работа с same-origin iframe через `browser_frames` и `frameIndex`; `frameIndex` выбирает same-origin iframe внутри текущего верхнего документа, а для доступа к верхнему документу параметр нужно опускать
